...
class ocr_recognition(dict):
	...
	def correction(self,pending_str):
		...
		if type(pending_str) != type(' '):
			print('不是字符串我不吃')
			return pending_str
		if pending_str in self.correct_dict:
			print('是的就是这样')
			return self.correct_dict.get(pending_str)
		else:
			return pending_str
...
def adb_test() :
	...
	"""
	if setting['basic']['connect_mode'] == 1 :
		try :
			message = os.popen(f'adb connect {0}'.format(setting['basic']['connect_aim'])).read()
			# print(message)
			if f'already connected to {0}'.format(setting['basic']['connect_aim']) in message :
				return True
